...
class Taxi:
    ...
    def update_schedule_distance(self, distance, load):
        self.schedule_distance = distance
        self.load += load
        self.num_order += 1


    def get_schedule_distance(self):
        return self.schedule_distance
    ...
    def satisfy_query(self, query):
        ...
        try:
            ...
            print self.serving_queries.keys()
            print query.id
            time.sleep(1000)
        self.num_riders -= 1
...
def gen_taxi(database, road_network):
    ...
    for i in database.grid:
        ...
        ## lyinr: taxi Number according to the ration of vertex in current grid
        if num_taxi == 0:
            continue
        # total_taxi += num_taxi

        ##lyinr: taxi Location on vertex location of current grid
        cnt = 0
